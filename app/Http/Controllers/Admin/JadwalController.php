<?php

namespace App\Http\Controllers\Admin;

use App\Http\Controllers\Controller;
use App\Http\Services\Summernote;
use App\Models\Jadwal;
use App\Models\Tahun;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\File;
use Yajra\DataTables\DataTables;


class JadwalController extends Controller
{
    public function index()
    {
        $tahun = Tahun::orderBy('id', 'desc')->get();
        return view('admin.jadwal.index', compact('tahun'));
    }

    public function data(Request $request)
    {
        $search = request('search.value');
        $data = Jadwal::join('tahun', 'tahun.id', '=', 'jadwal.tahun_id')
            ->select('jadwal.*', 'tahun.nama as tahun_nama');
        return DataTables::of($data)
            ->filter(function ($query) use ($search) {
                $query->where(function ($query) use ($search) {
                    $query->orWhere('jadwal.mulai', 'LIKE', "%$search%");
                    $query->orWhere('jadwal.berakhir', 'LIKE', "%$search%");
                    $query->orWhere('tahun.nama', 'LIKE', "%$search%");
                });
            })
            ->addColumn('action', function ($row) {
                $actionBtn = '
                <div class="dropdown">
                    <button class="btn btn-secondary dropdown-toggle" type="button"
                        id="dropdownMenuButton" data-toggle="dropdown"
                        aria-haspopup="true" aria-expanded="false">
                        Klik
                    </button>
                    <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                        <button type="button" class="dropdown-item"
                            data-toggle="modal" data-target="#modal_edit"
                            data-id="' . $row->id . '"
                            data-tahun_id="' . $row->tahun_id . '"
                            data-mulai="' . $row->mulai . '"
                            data-berakhir="' . $row->berakhir . '"
                        >Edit</button>
                        <form action="" onsubmit="deleteData(event)" method="POST">
                        ' . method_field('delete') . csrf_field() . '
                            <input type="hidden" name="id" value="' . $row->id . '">
                            <input type="hidden" name="nama" value="' . $row->mulai . '">
                            <button type="submit" class="dropdown-item text-danger">
                                Delete
                            </button>
                        </form>
                    </div>
                </div>';
                return $actionBtn;
            })
            ->rawColumns(['action'])
            ->toJson();
    }

    public function add(Request $request)
    {
        try {
            $request->validate([
                'tahun_id' => 'required',
                'mulai' => 'required',
                'berakhir' => 'nullable',
            ]);

            $mulai = strtolower($request->mulai);
            $berakhir = $request->berakhir;

            $jadwal = Jadwal::where([
                ['tahun_id', $request->tahun_id],
            ])->first();

            if ($jadwal) {
                $data = [
                    "message" => 500,
                    "data" => 'Data sudah ada, bisa diedit',
                    "req" => $request->all(),
                ];
                return $data;
            }

            $new = new Jadwal();
            $new->tahun_id = $request->tahun_id;
            $new->mulai = $mulai;
            $new->berakhir = $berakhir;
            $new->save();

            $data = [
                "message" => 200,
                "data" => 'Berhasil menambahkan Jadwal',
                "req" => $request->all(),
            ];
        } catch (\Throwable $th) {
            $data = [
                "message" => 500,
                "data" => $th->getMessage(),
                "req" => $request->all(),
            ];
        }
        return $data;
    }

    public function edit(Request $request)
    {
        try {
            $dataValidated = $request->validate([
                'id' => 'required',
                'tahun_id' => 'required',
                'mulai' => 'required',
                'berakhir' => 'nullable',
            ]);

            $id = $request->id;
            $tahunId = $request->tahun_id;
            $mulai = strtolower($request->mulai);
            $berakhir = $request->berakhir;

            $jadwal = Jadwal::where([
                ['tahun_id', $tahunId],
                ['id', '!=', $id]
            ])->first();

            if ($jadwal) {
                $data = [
                    "message" => 500,
                    "data" => 'Data sudah ada, bisa diedit',
                    "req" => $request->all(),
                ];
                return $data;
            }

            $edit = Jadwal::findOrFail($id);
            $edit->tahun_id = $tahunId;
            $edit->mulai = $mulai;
            $edit->berakhir = $berakhir;
            $edit->save();

            $data = [
                'message' => 200,
                'data' => 'Berhasil mengedit Jadwal',
                'req' => $request->all(),
            ];
        } catch (\Throwable $th) {
            $data = [
                'message' => 500,
                'data' => $th->getMessage(),
                'req' => $request->all(),
            ];
        }

        return $data;
    }

    public function delete(Request $request)
    {
        try {
            $dataValidated = $request->validate([
                'id' => 'required',
            ]);

            $jadwal = Jadwal::where('id', $dataValidated['id'])->first();

            $jadwal->destroy($dataValidated['id']);
            $data = [
                "message" => 200,
                "data" => "Berhasil menghapus data",
            ];
            return $data;
        } catch (\Throwable $th) {
            $data = [
                "message" => 500,
                "data" => 'Gagal menghapus data, seluruh data peserta dengan tipe yang sama harus dihapus dulu',
            ];
            return $data;
        }
    }
}