                ...
                <div class="row mb-2">
                    ...
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-right">
                            <li class="breadcrumb-item active">/ Kuota
                            </li>
                        </ol>
                    </div>
                </div>
                ...
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="form-group">
                                <label>Jenis</label>
                                <select class="form-control select2bs4 w-100" name="jenis" required>
                                    <option value="">PILIH JENIS</option>
                                    @foreach (\Helper::getEnumValues('kuota', 'jenis') as $item)
                            ...
                            <div class="form-group">
                                <label>Jenjang</label>
                                <select class="form-control select2bs4 w-100" name="jenjang" required>
                                    <option value="">PILIH JENJANG</option>
                                    @foreach (\Helper::getEnumValues('kuota', 'jenjang') as $item)
                                        <option value="{{ $item }}">{{ $item }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group">
                                <label>Kuota</label>
                                <input type="number" name="kuota" class="form-control" placeholder="Masukkan Kuota">
                            </div>
                        </div>
                ...
                <div class="modal-dialog modal-lg" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="form-group">
                                <label>Jenis</label>
                                <select class="form-control select2bs4 w-100" id="jenis" name="jenis"
                                    id="jenis" required>
                                    <option value="">PILIH BANIN/BANAT</option>
                                    @foreach (\Helper::getEnumValues('kuota', 'jenis') as $item)
                                        <option value="{{ $item }}">{{ $item }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group">
                                <label>Jenjang</label>
                                <select class="form-control select2bs4 w-100" name="jenjang" id="jenjang" required>
                                    <option value="">PILIH JENJANG</option>
                                    @foreach (\Helper::getEnumValues('kuota', 'jenjang') as $item)
                                        <option value="{{ $item }}">{{ $item }}</option>
                                    @endforeach
...
@push('script')
    ...
    <script>
        ...
        $('#modal_edit').on('show.bs.modal', function(event) {
            ...
            modal.find('#jenjang').val(jenjang).change();
            modal.find('#kuota').val(kuota);
        })

        $('#form_edit').submit(function(e) {
            e.preventDefault();

            var url = "{{ route('admin.kuota.edit') }}";
    ...
    <script>
        function deleteData(event) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    var url = "{{ route('admin.kuota.delete') }}";
        ...
        function dataTable(id) {
            var url = "{{ route('admin.kuota.data') }}"
